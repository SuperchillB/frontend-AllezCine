...
function imgMovie(jsonObj) {
  ...
  for (var i = 0; i < 4; i++) {
    ...
    // myId.dataset.toggle = 'modal';
    // myId.dataset.target = '#exampleModalCenter';
    //
    // carouselTrailerBtn.on('click', function() {
    //   for(let i = 0, length1 = item.length; i < length1; i++){
    //     for(let j = 0, length1 = film.length; j < length1; j++){
    //       if (item.text() == film[j].title) {
